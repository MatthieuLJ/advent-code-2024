...
fn main() {
    ...
    if heap1.len() != heap2.len() || heap1.len() == 0 {
        println!("I got {} and {} elements in the heaps, something went wrong", heap1.len(), heap2.len());
        process::exit(1);
    }
...
fn read_file_into_heaps(heap1: &mut BinaryHeap<u32>, heap2: &mut BinaryHeap<u32>) -> Result<(), std::io::Error> {
    for line in read_to_string("input.txt")?.lines() {
        let parts : Vec<&str> = line.split("   ").collect();
        if parts.len() != 2 {
            return Err(std::io::Error::new(std::io::ErrorKind::Other, "Did not get 2 parts"));
        }
